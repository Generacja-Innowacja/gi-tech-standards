...
your project. If you are an intern, please assign internship supervisors within the domain (frontend or backend).
3. **Update board and inform a team**: After assigning the reviewers, **developer** should write a message on project
Discord channel (`-back` or `-front`), and move task to `Code review` (`Waiting for review`, `In review` or similar) column in _GitHub Projects_.
...
for giving feedback. Please move the task to `Waiting for correction`
column if further changes are required.
